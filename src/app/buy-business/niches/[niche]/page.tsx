...
import {
  ...
  productEmojis as fallbackProductEmojis,
  nicheCtaLabel as fallbackNicheCtaLabel,
  getBrandAccent as fallbackGetBrandAccent,
  getBrandBadge as fallbackGetBrandBadge,
...
export default function NichePage({ params }: { params: Promise<{ niche: string }> }) {
  ...
  if (hasCms && cmsNiche) {
    /* CMS path */
    const brandsInNiche = (cmsBrands ?? [])
      .filter((b) => b.is_active && b.niche_id === cmsNiche.id)
      .sort((a, b) => a.sort_order - b.sort_order);

    const stats = (cmsStats ?? [])
      .filter((s) => s.niche_id === cmsNiche.id)
      .sort((a, b) => a.sort_order - b.sort_order);
    ...
    const emojis = getProductEmojis(cmsNiche.slug);
    const ctaLabel = getNicheCtaLabel(cmsNiche.slug);
    ...
    return (
      <main className="min-h-screen bg-black">
          ...
          <div className="max-w-7xl mx-auto px-6 w-full relative z-10">
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <span className="text-4xl">{cmsNiche.icon}</span>
                <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-heading font-black tracking-tight leading-[0.95]">
                  ...
                  <span className="text-amber">Businesses</span>
                </h1>
              </div>
              <p className="text-gray-400 text-sm sm:text-base md:text-lg font-medium leading-relaxed border-l-2 border-amber/30 pl-4 sm:pl-6">
                {resolveLocalized(cmsNiche.hero_description, langCode) ||
                  resolveLocalized(cmsNiche.description, langCode)}
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-6 space-y-16 md:space-y-24 pb-24 md:pb-32">
          {/* Why It Matters — niche_stats rows when present, otherwise a
              generic 3-row description block. */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-6 relative z-10">
              ...
              <h3 className="text-amber font-bold text-xl uppercase tracking-wider">
                Why {resolveLocalized(cmsNiche.title, langCode)}?
              </h3>
            </div>
            <p className="text-white text-sm sm:text-base font-medium leading-relaxed mb-6 relative z-10">
              {resolveLocalized(cmsNiche.description, langCode) ||
                `${resolveLocalized(cmsNiche.title, langCode)} businesses represent one of the most attractive acquisition opportunities in today's market.`}
            </p>
            {stats.length > 0 && (
          ...
          {/* What's Included — niche_inclusions rows when present. */}
          <div>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
          ...
          <div>
            ...
            >
              ...
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-black leading-tight tracking-tight mb-3">
                Available <span className="text-amber">
                  {resolveLocalized(cmsNiche.title, langCode)}
                </span>{" "}
                Brands
              </h2>
              <p className="text-gray-400 text-sm leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">
                {brandsInNiche.length} turnkey{" "}
                {resolveLocalized(cmsNiche.title, langCode).toLowerCase()} businesses currently
                available for acquisition.
              </p>
            </motion.div>
            {brandsInNiche.length === 0 ? (
              <p className="text-gray-500 text-sm">No brands published in this niche yet.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {brandsInNiche.map((brand, i) => {
                  const accent = getAccentFor(cmsNiche.slug, brand.slug);
                  const badge = getBadgeStyleFor(cmsNiche.slug, brand.slug);
                  const trustBadges = getTrustBadges(cmsNiche.slug);
                  const badgeClass = badgeClasses[badge] || "rounded-2xl";
                  const borderRadius =
                    badge === "glass"
                      ? "20px"
                      : badge === "square"
                    ...
                    >
                      ...
                      <div className="relative h-40 md:h-48 overflow-hidden">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div
                            className={`w-16 h-16 md:w-20 md:h-20 bg-black/60 backdrop-blur-sm border flex items-center justify-center group-hover:scale-110 transition-all duration-500 ${badgeClass}`}
                            style={{
                      ...
                      >
                        <h3 className="relative z-10 font-heading font-black text-2xl md:text-3xl text-white tracking-tight group-hover:text-white transition-colors duration-300">
                          {brand.name}
                        </h3>
                        <p className="relative z-10 text-gray-400 leading-relaxed text-xs sm:text-sm max-w-md mt-2 opacity-75">
                          {resolveLocalized(brand.description, langCode)}
                        </p>

                        <div className="relative z-10 flex flex-wrap justify-center gap-1.5 mt-4 mb-5">
                          {resolveLocalizedArray(brand.tags, langCode).map((tag) => (
                            <span
                              key={tag}
                        ...
                        <div className="relative z-10 flex items-center justify-center gap-3 flex-wrap w-full max-w-[320px]">
                          {trustBadges.slice(0, 4).map((badgeTxt) => (
                            <span
                              key={badgeTxt}
                      ...
                      <div className="border-t border-white/5">
                        <div className="px-5 md:px-6 pt-4 pb-3 flex items-center justify-between">
                          ...
                          <Link
                            href={`/buy-business/niches/${cmsNiche.slug}/${brand.slug}`}
          ...
          <div className="py-6 border-t border-white/10 text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-extrabold text-white mb-6 leading-tight">
              Ready to Acquire a {resolveLocalized(cmsNiche.title, langCode)} Business?
            </h2>
                ...
                .map((n) => (
                  ...
                  >
                    ...
                    <span className="text-white text-xs font-bold group-hover/card:text-amber transition-colors">
                      {resolveLocalized(n.title, langCode)}
                    </span>
                  </Link>
                ))}
            </div>
  ...
  /* ── Static-fallback branch (CMS has no niche with this slug yet) ── */
  const niche = fallbackNiche!; // narrowed above
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {niche.brands.map((brand, i) => {
              const accent = fallbackGetBrandAccent(brand, i);
              const badge = fallbackGetBrandBadge(brand, i);
              const badgeClass = badgeClasses[badge] || "rounded-2xl";
              const emojis = fallbackProductEmojis[niche.title] || ["📦", "⭐", "🔧"];
              const ctaLabel = fallbackNicheCtaLabel[niche.title] || "Learn More →";
              return (
                <motion.div
                  key={brand.slug}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div
                        ...
                        style={{
                          borderColor: accent + "40",
                          boxShadow: `0 0 30px ${accent}10`,
                          borderRadius: badge === "glass" ? "20px" : badge === "square" ? "12px" : "20px",
                        }}
                      >
                        <span className="text-3xl md:text-4xl">{brand.logo}</span>
                      </div>
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-center justify-center gap-3 flex-wrap w-full max-w-[320px]">
                      {brand.trustBadges.slice(0, 4).map((badgeTxt) => (
                        <span
                          key={badgeTxt}
                          className="flex items-center gap-1 text-[9px] sm:text-[10px] text-gray-500 font-medium"
                        >
