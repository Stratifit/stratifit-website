...
import { HiArrowRight, HiArrowLeft, HiCheck, HiSparkles, HiGlobeAlt, HiShieldCheck } from "react-icons/hi2";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { getNicheBySlug, niches, productEmojis, nicheCtaLabel, getBrandAccent, getBrandBadge } from "@/data/buy-business";
  ...
  return (
    <main className="min-h-screen bg-black">
        ...
        <div className="max-w-7xl mx-auto px-6 w-full relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
            <div className="mb-4 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10">
              ...
              <span className="text-[10px] font-bold tracking-widest uppercase text-gray-400">Business Acquisition</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-16 md:space-y-24 pb-24 md:pb-32">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-card-dark rounded-2xl p-6 sm:p-8 shadow-lg border border-white/5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2 mb-6 relative z-10">
            ...
            <h3 className="text-amber font-bold text-xl uppercase tracking-wider">Why {niche.title}?</h3>
          </div>
          <p className="text-white text-sm sm:text-base font-medium leading-relaxed mb-6 relative z-10">
            {niche.title} businesses represent one of the most attractive acquisition opportunities in today&apos;s market. With proven business models, established revenue streams, and significant growth potential, these assets offer a faster path to ownership than building from scratch.
          </p>
        ...
        <div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-8">
            ...
            <p className="text-gray-400 text-sm leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">Every acquisition includes everything you need to hit the ground running from day one.</p>
          </motion.div>
        ...
        <div>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-8">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-black leading-tight tracking-tight mb-3">Available <span className="text-amber">{niche.title}</span> Brands</h2>
            <p className="text-gray-400 text-sm leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">{niche.brands.length} turnkey {niche.title.toLowerCase()} businesses currently available for acquisition.</p>
          </motion.div>
              ...
              >
                ...
                <div className="bg-[#1a1a1a] px-4 py-2.5 flex items-center gap-2 border-b border-white/5">
                  ...
                  <div className="flex-1 mx-3 bg-[#0d0d0d] rounded-md px-3 py-1.5 flex items-center gap-2 border border-white/5">
                    ...
                    <span className="text-[10px] text-gray-500 truncate">{brand.websiteUrl.replace("https://", "")}</span>
                  </div>
                  <span className="text-[9px] font-bold px-2 py-0.5 shrink-0 uppercase tracking-wider" style={{ color: accent, background: accent + "18", borderColor: accent + "30", border: "1px solid", borderRadius: badge === "square" ? "6px" : "9999px" }}>
                    {brand.niche}
                  </span>
                </div>
                ...
                <div className="bg-[#141414] px-5 py-2 flex items-center justify-between border-b border-white/5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-bold text-gray-300 ml-0.5">{brand.name}</span>
                  </div>
                  <div className="flex items-center gap-3 sm:gap-4">
                    <span className="text-[8px] sm:text-[9px] text-gray-600 font-medium uppercase tracking-wider" aria-hidden="true">Home</span>
                    <span className="text-[8px] sm:text-[9px] text-gray-600 font-medium uppercase tracking-wider" aria-hidden="true">About</span>
                    <span className="text-[8px] sm:text-[9px] text-gray-600 font-medium uppercase tracking-wider" aria-hidden="true">Products</span>
                    <span className="text-[8px] sm:text-[9px] text-amber/60 font-medium uppercase tracking-wider" aria-hidden="true">Contact</span>
                  </div>
                </div>
                ...
                <div className="relative h-40 md:h-48 overflow-hidden">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className={`w-16 h-16 md:w-20 md:h-20 bg-black/60 backdrop-blur-sm border flex items-center justify-center group-hover:scale-110 transition-all duration-500 ${badgeClasses[badge] || "rounded-2xl"}`} style={{ borderColor: accent + "40", boxShadow: `0 0 30px ${accent}10`, borderRadius: badge === "glass" ? "20px" : badge === "square" ? "12px" : "20px" }}>
                ...
                >
                  {/* Brand Name */}
                  <h3 className="relative z-10 font-heading font-black text-2xl md:text-3xl text-white tracking-tight group-hover:text-white transition-colors duration-300" style={{ color: "inherit" }}>
                    {brand.name}
                  </h3>
                  {/* Description */}
                  ...
                  <div className="relative z-10 w-full max-w-[320px] mb-5">
                    <div className="grid grid-cols-3 gap-2">
                      {emojis.map((emoji, idx) => (
                        ...
                        >
                          <span className="text-2xl opacity-35 group-hover:opacity-55 transition-opacity">{emoji}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className="py-6 border-t border-white/10 text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-extrabold text-white mb-6 leading-tight">Ready to Acquire a {niche.title} Business?</h2>
          ...
          <a href="#contact" className="group inline-flex w-full sm:w-auto px-8 sm:px-12 py-4 bg-amber hover:bg-amber-light text-black text-lg font-bold rounded-full shadow-[0_0_30px_-5px_rgba(245,158,11,0.4)] transition-all hover:scale-[1.02] active:scale-[0.98] items-center justify-center gap-2">
            Start the Acquisition Process
            <HiArrowRight className="group-hover:translate-x-1 transition-transform" />
          </a>
          <p className="text-gray-500 text-xs mt-4 font-medium">Limited availability — new listings added weekly</p>
        </div>

        {/* Explore Other Niches */}
        <div className="border-t border-white/10 pt-12">
          <h3 className="text-white font-heading font-bold text-2xl mb-6 text-center">Explore Other <span className="text-amber">Niches</span></h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {niches.filter(n => n.slug !== nicheSlug).map((n) => (
              <Link key={n.slug} href={`/buy-business/niches/${n.slug}`} className="bg-card-dark rounded-xl p-4 border border-white/5 hover:border-amber/20 transition-all text-center group/card">
                <span className="text-2xl block mb-2">{n.icon}</span>
                <span className="text-white text-xs font-bold group-hover/card:text-amber transition-colors">{n.title}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <motion.button
        ...
        onClick={() => { if (window.history.length > 1) { router.back(); } else { router.push("/buy-business"); } }}
