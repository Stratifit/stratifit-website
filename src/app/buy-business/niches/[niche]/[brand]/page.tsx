...
import {
  ...
  getNicheBySlug as fallbackGetNicheBySlug,
  getBrandBySlug as fallbackGetBrandBySlug,
...
export default function BrandDetailPage({
  ...
  params: Promise<{ niche: string; brand: string }>;
}) {
  ...
  const { data: cmsInclusions } = useCms<NicheInclusion[]>("niche-inclusions", {
    fallback: [],
  });

  const cmsNiche = (cmsNiches ?? []).find((n) => n.slug === nicheSlug);
  const cmsBrand = cmsNiche
    ? (cmsBrands ?? []).find(
        (b) => b.niche_id === cmsNiche.id && b.slug === brandSlug,
      )
    : undefined;

  const hasCms = Boolean(cmsNiche && cmsBrand);

  /* ── Static fallback branch (only used when CMS has no rows yet) ── */
  const fallbackNiche = hasCms ? undefined : fallbackGetNicheBySlug(nicheSlug);
  const fallbackBrand = hasCms
    ? undefined
    : fallbackNiche
      ? fallbackGetBrandBySlug(brandSlug)
      : undefined;

  if (!hasCms && (!fallbackNiche || !fallbackBrand)) {
    return (
  ...
  /* ── Render: CMS path ──────────────────────────────────────── */
  if (hasCms && cmsNiche && cmsBrand) {
    const accent = getAccentFor(cmsNiche.slug, cmsBrand.slug);
    const inclusions = (cmsInclusions ?? [])
      .filter((i) => i.niche_id === cmsNiche.id)
      .sort((a, b) => a.sort_order - b.sort_order);
    const trustBadges = getTrustBadges(cmsNiche.slug);
        ...
        <section className="relative min-h-[60vh] flex items-center pt-24 pb-8 md:pt-32">
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            <div
              ...
              style={{ backgroundImage: `url(${cmsBrand.image})` }}
            />
          ...
          <div className="max-w-7xl mx-auto px-6 w-full relative z-10">
            ...
            >
              <div
                ...
                style={{ background: accent + "20", borderColor: accent + "40", border: "1px solid" }}
              >
                <span
        ...
        <div className="max-w-7xl mx-auto px-6 space-y-16 md:space-y-24 pb-24 md:pb-32">
          ...
          >
            {trustBadges.slice(0, 4).map((badge) => (
              <div
                key={badge}
                className="bg-card-dark rounded-xl p-4 border border-white/5 flex flex-col items-center text-center gap-1.5"
              >
          ...
          <div>
            ...
            >
              ...
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-black leading-tight tracking-tight mb-3">
                How It&apos;s <span className="text-amber">Built</span>
              </h2>
              <p className="text-gray-400 text-sm leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">
                The technical foundation and operational setup behind {cmsBrand.name}.
              </p>
            </motion.div>
              ...
              {[
                {
                  icon: HiCog,
                  title: "Tech Stack",
                  desc: `Built on modern, scalable infrastructure with documented architecture and deployment pipelines. Ready for handoff with full source code access.`,
                },
                {
                  icon: HiLightBulb,
                  title: "Business Model",
                  desc: `Proven revenue model with ${resolveLocalizedArray(cmsBrand.highlights, langCode)[0] || "established metrics"}. All customer relationships and contracts transfer with the sale.`,
                },
                {
                  icon: HiTruck,
                  title: "Operations",
                  desc: `Standard operating procedures documented. Team structure and vendor relationships in place for seamless transition to new ownership.`,
                },
              ].map((item) => (
                <div
                  key={item.title}
                  className="bg-card-dark rounded-xl p-5 border border-white/5 hover:border-amber/20 transition-all flex flex-col items-center text-center gap-3"
                >
          ...
          <div>
            ...
            >
              ...
              <p className="text-gray-400 text-sm leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">
                Everything that transfers with the acquisition of {cmsBrand.name}.
              </p>
            </motion.div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {inclusions.map((inc, i) => (
                ...
                >
                  ...
                  <span className="text-white text-sm">{resolveLocalized(inc.text, langCode)}</span>
                </motion.div>
              ))}
              {inclusions.length === 0 && (
                <p className="text-gray-500 text-sm col-span-full">
                  Inclusion list not yet published.
                </p>
              )}
            </div>
          </div>
          ...
          {/* Highlights */}
          {resolveLocalizedArray(cmsBrand.highlights, langCode).length > 0 && (
            <div>
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-black leading-tight tracking-tight mb-3">
                  Key <span className="text-amber">Highlights</span>
                </h2>
              </motion.div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {resolveLocalizedArray(cmsBrand.highlights, langCode).map((h, i) => (
                  <div
                    key={h}
                    className="bg-card-dark rounded-xl p-5 border border-white/5 hover:border-amber/20 transition-all flex items-center gap-3"
                  >
          ...
          <div className="bg-card-dark rounded-2xl p-6 sm:p-8 shadow-lg border border-white/5 relative overflow-hidden">
            ...
            <p className="text-white text-sm sm:text-base font-medium leading-relaxed mb-6 relative z-10">
              We handle the entire acquisition process from due diligence to final handover.
              Here&apos;s how it works:
            </p>
              ...
              {[
                {
                  step: "1",
                  title: "Due Diligence",
                  desc: "Full access to financial records, codebase audits, and operational metrics.",
                },
                {
                  step: "2",
                  title: "Escrow & Payment",
                  desc: "Funds held securely in escrow until all transfer conditions are met.",
                },
                {
                  step: "3",
                  title: "Asset Transfer",
                  desc: "All accounts, domains, code, and intellectual property transferred to you.",
                },
                {
                  step: "4",
                  title: "Transition Support",
                  desc: "Ongoing support to ensure smooth operations under new ownership.",
                },
              ].map((s) => (
                <div
                  key={s.step}
                  className="flex items-start gap-4 bg-[#1E1E1E] rounded-xl p-4 border border-white/5"
                >
          ...
          <div className="py-6 border-t border-white/10 text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-extrabold text-white mb-6 leading-tight">
              Ready to Acquire {cmsBrand.name}?
            </h2>
            <div className="flex items-center justify-center gap-2 mb-4">
              <span className="font-heading font-black text-amber text-2xl">{cmsBrand.price}</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group inline-flex sm:w-auto px-8 py-4 border border-white/10 text-white text-base font-bold rounded-full hover:border-amber/30 transition-all items-center justify-center gap-2"
              >
                <HiGlobeAlt />
                Visit Site
              </a>
          ...
          <div className="border-t border-white/10 pt-12">
            <h3 className="text-white font-heading font-bold text-2xl mb-6 text-center">
              More <span className="text-amber">
                {resolveLocalized(cmsNiche.title, langCode)}
              </span>{" "}
              Businesses
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {(cmsBrands ?? [])
                .filter((b) => b.niche_id === cmsNiche.id && b.slug !== brandSlug && b.is_active)
                .slice(0, 4)
                .map((b) => (
                  <Link
                    key={b.id}
                    href={`/buy-business/niches/${nicheSlug}/${b.slug}`}
        ...
        <motion.button
          ...
          onClick={() => {
            if (window.history.length > 1) {
              ...
              router.push(`/buy-business/niches/${nicheSlug}`);
            }
          }}
  ...
  /* ── Render: static-fallback branch ─────────────────────────── */
  const niche = fallbackNiche!;
      ...
      <div className="max-w-7xl mx-auto px-6 space-y-16 md:space-y-24 pb-24 md:pb-32">
        ...
        <div>
          ...
          >
            ...
            <p className="text-gray-400 text-sm leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">
              The technical foundation and operational setup behind {brand.name}.
            </p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              },
              {
                icon: HiLightBulb,
                title: "Business Model",
                desc: `Proven revenue model with ${brand.highlights[0] || "established metrics"}. All customer relationships and contracts transfer with the sale.`,
              },
              {
                icon: HiTruck,
                title: "Operations",
        ...
        <div>
          ...
          >
            ...
            <p className="text-gray-400 text-sm leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">
              Everything that transfers with the acquisition of {brand.name}.
            </p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="border-t border-white/10 pt-12">
          <h3 className="text-white font-heading font-bold text-2xl mb-6 text-center">
            More <span className="text-amber">{niche.title}</span> Businesses
          </h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {niche.brands
