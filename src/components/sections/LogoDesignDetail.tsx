...
import {
  ...
  HiPencilSquare, HiCube, HiCheckBadge,
  HiStar, HiCheckCircle, HiChevronUp, HiChevronDown,
  HiBolt, HiDocumentText, HiEye, HiPhoto, HiArrowLeft,
} from "react-icons/hi2";

const phases = [
  { step: "STEP 01", icon: HiMagnifyingGlass, title: "Discovery & Research", phase: "Exploration Phase", desc: "Deep dive into your brand ethos, industry landscape, and visual competitors.", items: ["Brand Immersion","Visual Audit","Moodboarding","Inspiration Curation"] },
  { step: "STEP 02", icon: HiPencilSquare, title: "Concept & Sketching", phase: "Ideation Phase", desc: "Translating strategy into visual concepts through iterative sketching and exploration.", items: ["Thumbnail Sketches","Concept Routes","Typography Pairs","Symbol Exploration"] },
  { step: "STEP 03", icon: HiCube, title: "Refinement & Vectoring", phase: "Precision Phase", desc: "Polishing the chosen direction into a flawless, scalable vector mark.", items: ["Grid Refinement","Color Iteration","Scale Testing","Context Mockups"] },
  { step: "STEP 04", icon: HiCheckBadge, title: "Delivery & Guidelines", phase: "Finalization Phase", desc: "Delivering the complete logo system with comprehensive usage guidelines.", items: ["Master Files","Color Variants","Minimum Sizes","Usage Rules"] },
];

const timeline = [{ w:"WEEK 1", t:"Discovery"},{ w:"WEEK 2", t:"Sketching"},{ w:"WEEK 3", t:"Refinement"},{ w:"WEEK 4", t:"Delivery"}];

const deliverables = [
  { icon: HiPhoto, title:"Primary Logo", desc:"Full-color master mark in vector & raster." },
  { icon: HiCube, title:"Logo Variants", desc:"Horizontal, vertical, icon-only & monochrome." },
  { icon: HiEye, title:"Brandmark System", desc:"Sub-marks, patterns & secondary elements." },
  { icon: HiDocumentText, title:"Usage Guide", desc:"Clear rules for consistent application." },
];

const faqs = [
  { q:"How many logo concepts do I receive?", a:"We present 3 distinct concept directions, each with rationale. You select one for refinement into the final mark." },
  { q:"Do I own the final logo?", a:"Yes. Upon final payment, you receive full ownership and all source files (AI, EPS, SVG, PNG)." },
  { q:"How many revision rounds?", a:"Two rounds of refinements on the chosen concept are included. Additional rounds available if needed." },
  { q:"What file formats do I get?", a:"AI (editable vector), EPS, SVG, PNG (transparent), and JPG in all color variants." },
];
    ...
    <section className="pt-2 pb-24 md:pt-4 md:pb-32">
      <div className="max-w-7xl mx-auto px-6 space-y-16 md:space-y-24">
          ...
          <div className="w-full relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
              ...
              <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-heading font-black tracking-tight leading-[0.95] mb-6 uppercase">
                Deep Dive<br/><span className="text-amber">Logo Design</span>
              </h1>
              <p className="text-gray-400 text-sm sm:text-base md:text-lg font-medium leading-relaxed border-l-2 border-amber/30 pl-4 sm:pl-6">
                Crafting a memorable, scalable, and timeless mark that becomes the cornerstone of your brand's entire visual identity.
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-card-dark rounded-2xl p-6 sm:p-8 shadow-lg border border-white/5 relative overflow-hidden">
          ...
          <p className="text-gray-300 text-sm sm:text-base font-medium leading-relaxed relative z-10">Your logo is the face of your brand. It's the first impression, the memorable mark that customers recall. A professionally crafted logo communicates credibility, differentiation, and emotional connection before a single word is read.</p>
        </motion.div>
        ...
        <div className="bg-card-dark rounded-2xl border border-white/5 p-6 sm:p-8 shadow-2xl relative overflow-hidden"><div className="absolute top-0 right-0 w-24 h-24 bg-amber/5 rounded-full blur-2xl"></div><div className="flex items-start gap-4 mb-4"><div className="w-10 h-10 rounded-full bg-gradient-to-br from-amber to-orange-500 text-black font-black flex items-center justify-center text-sm shadow-lg shrink-0">MK</div><div><div className="flex gap-1 mb-1">{[...Array(5)].map((_,j)=>(<HiStar key={j} className="text-amber text-sm" />))}</div><h4 className="text-white font-bold text-sm">Michael Kim</h4><p className="text-[10px] text-gray-500 uppercase font-bold tracking-wide">Creative Director, Aura</p></div></div><p className="text-gray-300 text-sm italic font-medium leading-relaxed relative z-10">"The logo Stratifit designed for us didn't just look beautiful—it told our story at a glance. The systematic approach to variants made implementation seamless across our entire product ecosystem."</p></div>
        ...
        <div className="py-6 border-t border-white/10 text-center"><h2 className="text-2xl sm:text-3xl md:text-4xl font-heading font-extrabold text-white mb-6 leading-tight">Ready for an Iconic Mark?</h2><a href="#contact" className="inline-flex w-full sm:w-auto px-8 sm:px-12 py-4 bg-amber hover:bg-amber-light text-black text-lg font-bold rounded-full shadow-[0_0_30px_-5px_rgba(245,158,11,0.4)] transition-all hover:scale-[1.02] active:scale-[0.98] items-center justify-center gap-2 group">Start a Logo Project<HiArrowRight className="group-hover:translate-x-1 transition-transform" /></a><p className="text-gray-500 text-xs mt-4 font-medium">Limited availability for Q3 2026</p></div>
      </div>
