...
import { HiArrowRight, HiArrowLeft, HiGlobeAlt, HiShieldCheck } from "react-icons/hi2";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          ...
          <p className="text-gray-400 text-sm sm:text-base md:text-lg leading-relaxed max-w-2xl border-l-2 border-amber/50 pl-4 sm:pl-6 mt-3">
            Skip the startup grind. Browse turnkey businesses with real revenue, existing customers,
          ...
          <div className="text-center py-16">
            <div className="inline-flex items-center gap-2 bg-card-dark border border-white/5 rounded-2xl px-8 py-6">
              <p className="text-gray-400 text-sm font-medium">
                No businesses found in <span className="text-amber font-bold">{activeFilter}</span>
              </p>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div className="md:hidden flex items-center justify-center gap-1.5 mt-3">
              {filteredBrands.map((_, idx) => (
                <div
                  key={idx}
                  className={`rounded-full transition-all duration-200 ease-out ${idx === activeIndex ? "w-3 h-1.5 bg-amber" : "w-1.5 h-1.5 bg-white/20"}`}
                />
              ))}
            </div>
            <div className="md:hidden flex justify-center mt-2">
              <Link
                href="/buy-business"
                className="inline-flex items-center gap-1.5 text-amber text-xs font-bold uppercase tracking-wider hover:text-amber-light transition-colors"
              >
